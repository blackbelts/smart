...
export class ChoicePage {
  ...
  constructor(
    ...
    this.moodelProvider.getChoiceByCourse( this.navParams.get("cId"))
      .map(res => res)
      .subscribe((couresChoices) => {
        console.log(couresChoices)
        for (let i = 0; i < couresChoices.choices.length; i++) {
          if (couresChoices.choices[i].id == this.id)
            this.setChoiceInfo(couresChoices.choices[i])
  ...
  setChoiceInfo(info) {
    ...
    console.log(this.choiceInfo.intro)
  }
